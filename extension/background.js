...
chrome.runtime.onConnect.addListener((port) => {
  ...
  port.onDisconnect.addListener(() => {
    ...
    if (current && current.port === port) {
      ...
      closeSessionSocket(current, "content script disconnected");
      updateSessionState(current);
      setTabEnabled(tabId, false).catch((error) => {
        console.error("[web-model:bg] clear tab enabled state failed", `tab=${tabId}`, error);
      });
    }
  });
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  if (message?.type === "config.updated") {
    onConfigUpdated().then(async () => {
      ...
      sendResponse(buildStatus(config, tabId));
    }).catch((error) => {
...
async function connectTab(tabId, reason = "auto connect") {
  ...
  session.socket = new WebSocket(config.serverUrl);
...
function buildStatus(config, tabId) {
  if (typeof tabId !== "number") {
    return {
      ok: false,
