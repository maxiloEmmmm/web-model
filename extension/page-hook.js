...
(function installWebModelPageHook() {
  ...
  const fallbackParser = {
    ...
    extractError(payload) {
      ...
      function pickError(node) {
        ...
        if (typeof node.code === "string" && normalizeText(node.code)) {
          return normalizeText(node.code);
        }
        return "";
      }
  ...
  function extractProviderError(payload, providerType) {
    const parser = getProviderParser(providerType);
    if (typeof parser.extractError === "function") {
      return parser.extractError(payload);
    }
    return fallbackParser.extractError(payload);
  }
  ...
  function parseSSEBlock(block, providerType, state) {
    ...
    if (data === "[DONE]") {
      state.logicalDone = true;
      return;
    }

    try {
      const payload = JSON.parse(data);
      const providerError = extractProviderError(payload, providerType);
      if (providerError) {
  ...
  function parseKimiFramePayload(frameText, state) {
    try {
      ...
      if (
        String(payload?.mask || "") === "message" &&
        String(payload?.message?.role || "") === "assistant" &&
        typeof payload?.message?.id === "string" &&
        payload.message.id
      ) {
        state.kimiAssistantMessageId = payload.message.id;
      }
      if (
        String(payload?.mask || "") === "message.status" &&
        String(payload?.message?.status || "") === "MESSAGE_STATUS_COMPLETED" &&
        typeof payload?.message?.id === "string" &&
        payload.message.id &&
        payload.message.id === state.kimiAssistantMessageId
      ) {
        state.logicalDone = true;
      }
      if (
      ...
      const providerError = extractProviderError(payload, "kimi");
      if (providerError) {
        state.error = providerError;
        state.logicalDone = true;
        return;
  ...
  function emitStreamingDone(state) {
  ...
  if (originalXHROpen && originalXHRSend) {
    ...
    window.XMLHttpRequest.prototype.send = function webModelSend(body) {
      ...
      this.addEventListener("progress", () => {
        ...
        if (hook.streamingState.logicalDone) {
          finalizeStreamingState(hook.streamingState);
          emitStreamingDone(hook.streamingState);
        }
      });
      ...
      this.addEventListener("error", () => {
        if (hook.streamingState.doneEmitted) {
          return;
        }
        log("capture error", {
          providerType: hook.capture.providerType,
