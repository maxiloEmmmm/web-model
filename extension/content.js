...
(async function bootstrap() {
  ...
  function notifyWatchersByCapture(captureId) {
    if (!captureId) {
      return;
    }
    for (const watcher of pageHookState.watchers.values()) {
      if (watcher.captureId !== captureId) {
        continue;
      }
  ...
  function cleanupWatcher(requestId, watcher) {
    if (!watcher) {
    ...
    if (watcher.captureId) {
      pageHookState.captures.delete(watcher.captureId);
      watcher.captureId = "";
    }
    pageHookState.watchers.delete(requestId);
  }
  ...
  function onPageHookPayload(payload) {
    ...
    if (payload.type === "capture.start") {
      ...
      for (const watcher of pageHookState.watchers.values()) {
        ...
        if (!watcher.submitReadyAt || Date.now() + 50 < watcher.submitReadyAt) {
          continue;
        }
        if (
          payload.prompt &&
    ...
    if (payload.type === "capture.update") {
      ...
      notifyWatchersByCapture(payload.captureId);
  ...
  function createNetworkCapture(providerType, prompt, requestId) {
    ...
    for (const [existingRequestID, existingWatcher] of pageHookState.watchers.entries()) {
      if (existingWatcher.providerType !== providerType) {
        continue;
      }
      cleanupWatcher(existingRequestID, existingWatcher);
      log("network capture watcher replaced", {
        requestId,
        replacedRequestId: existingRequestID,
        providerType
      });
    }
    ...
    return {
      arm() {
        watcher.submitReadyAt = Date.now();
      },
      ...
      async waitForResult(timeoutMs = 120000) {
        ...
        while (Date.now() < deadline) {
          ...
          if (capture?.error) {
            cleanupWatcher(requestId, watcher);
            throw new Error(capture.error);
          }
          if (capture?.done) {
            if (!capture.text) {
              log("network capture empty, keep waiting", {
                requestId,
                captureId: watcher.captureId,
                providerType,
                promptPreview: clipText(prompt || ""),
                error: capture.error || ""
              });
              pageHookState.captures.delete(watcher.captureId);
              watcher.captureId = "";
