(function registerKimiProvider(globalScope) {
  ...
  function createKimiPageAdapter(log) {
    const KIMI_USER_MESSAGE_SELECTOR = ".chat-content-item-user .user-content";
    const KIMI_ASSISTANT_MESSAGE_SELECTOR = ".chat-content-item-assistant .markdown-container .markdown";
    const KIMI_COMPOSER_SELECTOR = "div.chat-input-editor[data-lexical-editor='true'][contenteditable='true'][role='textbox']";
    const KIMI_SEND_BUTTON_SELECTOR = ".send-button-container";
    const KIMI_NEW_CHAT_SELECTOR = "a.new-chat-btn[href*='chat_enter_method=new_chat']";
    ...
    return {
      ...
      async startNewChat() {
        const trigger = document.querySelector(KIMI_NEW_CHAT_SELECTOR);
        if (!(trigger instanceof Element) || !isVisible(trigger)) {
          throw new Error("kimi new chat trigger not found");
        }
        log("start new chat", {
      ...
      async waitForComposer(timeoutMs = 15000) {
        ...
        throw new Error("kimi composer not found");
      },

      findComposer() {
        const node = document.querySelector(KIMI_COMPOSER_SELECTOR);
      ...
      async submitComposer(node, button = null) {
        ...
        for (let attempt = 0; attempt < 5; attempt += 1) {
          ...
          if (this.isSendButtonClickable(currentButton)) {
            log("submit fallback via button", currentButton.className || currentButton.tagName);
      ...
      findSubmitButton() {
        const button = document.querySelector(KIMI_SEND_BUTTON_SELECTOR);
        return button instanceof Element && isVisible(button) ? button : null;
      },
      ...
      async waitForAssistantReply(requestId, composer, initialSubmitButton, prompt, baseline) {
        ...
        const waitRoot = document.querySelector(".chat-content-item-assistant")?.parentElement || composer?.closest?.("main") || document.body;
      ...
      listAssistantMessages() {
        return Array.from(document.querySelectorAll(KIMI_ASSISTANT_MESSAGE_SELECTOR))
      ...
      readLastUserPrompt() {
        const items = Array.from(document.querySelectorAll(KIMI_USER_MESSAGE_SELECTOR))
          .filter((node) => isVisible(node))
      ...
      describeSubmitControl(button) {
        ...
        const isBusy = /\bstop\b/i.test(className);
        const isDisabled = /\bdisabled\b/i.test(className);

        return {
          present: true,
          mode: isBusy ? "busy" : isDisabled ? "disabled" : "ready",
  ...
  registerProvider({
    type: "kimi",
    displayName: "Kimi",
    description: "Kimi bridge driven by the local web-model extension.",
    homepage: "https://www.kimi.com/",
    hostPatterns: ["https://www.kimi.com/*"],
    matchesURL(url) {
      return /^https:\/\/www\.kimi\.com\/.*/.test(String(url || ""));
    },
    buildTabKey(tabId) {
      return `kimi-tab-${tabId}`;
    },
    buildAutoKey(sequence) {
      return `kimi-${sequence}`;
    },
    createPageAdapter(log) {
      return createKimiPageAdapter(log);
    }
  });
